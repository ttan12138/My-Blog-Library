...
const routes = [
  //配置路由和组件之间的对应关系
  {
    ...
    name: 'Home',
    component: () => import('components/Home')  // 组件名
  },
  {
...
### 2.2.3 使用路由：通过`<router-link>`和`<router-view>`
...
使用`npm run dev`启动项目，此时`<router-view>`在`<router-link>`下面，那渲染页面就在下面的。

![image-20200903163216310](C:\Users\ttan12138\AppData\Roaming\Typora\typora-user-images\image-20200903163216310.png)

 当把`<router-view>`置于`<router-link>`上面，那么渲染页面就在上面。

![image-20200903163121391](C:\Users\ttan12138\AppData\Roaming\Typora\typora-user-images\image-20200903163121391.png)

### 2.2.4 路由的默认值和history模式
...
![image-20200903163420372](C:\Users\ttan12138\AppData\Roaming\Typora\typora-user-images\image-20200903163420372.png)



### 2.2.5 `<router-link>`的其他属性
